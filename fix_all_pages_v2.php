<?php
/**
 * سكربت إصلاح جميع صفحات PHP - النسخة المحسنة
 */
...
if (isset($_GET['get_files'])) {
    ...
    $excluded_files = ['config.php', 'database.php', 'functions.php', 'permissions.php', 'fix_all_pages.php', 'fix_all_pages_v2.php', 'debug_files.php', 'install.php', 'setup.php'];
...
function fixPHPFileV2($filepath) {
    ...
    if (preg_match('/<html[^>]*>/i', $content, $matches)) {
        ...
        // إضافة HTML tag إذا لم يكن موجود
        ...
        $modified = true;
        $changes[] = 'إضافة HTML كامل';
    }
    
    // 2. إصلاح Bootstrap CSS
    ...
    // 3. إصلاح Bootstrap 5.1.3 و 5.3.0
    if (preg_match('/bootstrap@5\.(1\.3|3\.0)\/dist\/css\/bootstrap\.min\.css/i', $content)) {
        $content = preg_replace(
            '/bootstrap@5\.(1\.3|3\.0)\/dist\/css\/bootstrap\.min\.css/i',
            'bootstrap@5.$1/dist/css/bootstrap.rtl.min.css',
            $content
        );
        $modified = true;
        $changes[] = 'تحديث Bootstrap RTL';
...
<head>
    ...
    <title>إصلاح جميع صفحات المشروع - النسخة المحسنة</title>
...
<body>
    <div class="container">
        <h1><i class="fas fa-tools"></i> إصلاح جميع صفحات المشروع - النسخة المحسنة</h1>
        
        <div class="warning">
            <h4><i class="fas fa-exclamation-triangle"></i> تحذير!</h4>
            <p>هذا السكربت المحسن سيقوم بإصلاح جميع ملفات PHP التي تحتوي على HTML.</p>
        </div>
        
        <div id="startSection">
            <div class="d-grid gap-2">
                <button type="button" id="startBtn" class="btn btn-primary btn-lg">
                    <i class="fas fa-play me-2"></i>بدء الإصلاح المحسن
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        document.getElementById('startBtn').addEventListener('click', function() {
            if (confirm('هل أنت متأكد من تشغيل السكربت المحسن؟')) {
                startProcessing();
            }
        });
        ...
        async function startProcessing() {
            ...
            try {
                const response = await fetch('fix_all_pages_v2.php?get_files=1');
        ...
        async function processFile(filepath) {
            try {
                ...
                const response = await fetch('fix_all_pages_v2.php', {
                    method: 'POST',
                    body: formData
                });
